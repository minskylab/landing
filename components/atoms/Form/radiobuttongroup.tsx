...
const RadioButtonGroup: FC<RadioButtonGroupProps & React.AllHTMLAttributes<any>> = (
    props: RadioButtonGroupProps & React.AllHTMLAttributes<any>
) => {
    type Check = {
        value: string;
        active: boolean;
    ...
    return (
        <CustomButtonGroup {...props}>
            ...
            {newRadioButton.map((value, index, arr) => {
                return (
                    <Wrapper {...props}>
                        <CustomRadioButton {...props} onClick={() => click(value.value)}>
                            {" "}
                            {value.selected ? <Checked {...props} /> : ""}{" "}
                        </CustomRadioButton>
                        <Text onClick={() => click(value.value)} {...props}>
                            {value.text || ""}
                        </Text>{" "}
                    </Wrapper>
                );
